...
public class checkIntegrity {
    public static boolean checkIntegrity() throws IOException {
        ...
        String url = "https://api.hackerman.tech/public/integrity.php?username=" + Emulator.getConfig().getValue("hh.username") + "&email=" + Emulator.getConfig().getValue("hh.email") + "&plugin=" + productId + "&version=" + version + "&url=" + Emulator.getConfig().getValue("hh.hotel_url");
        try {
            URL obj = new URL ( url );
            ...
            switch (status) {
                ...
                case "BLOCK":
                    ...
                    if (type.equalsIgnoreCase("BAN")) { // Used in extreme cases if the user breached the TOS / Morningstar License
